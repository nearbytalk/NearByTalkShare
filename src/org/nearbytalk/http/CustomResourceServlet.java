 ...
 * servlet itself or as ServletContext initParameters with a prefix of
 ...
 * 
 */
public class CustomResourceServlet extends HttpServlet implements
		ResourceFactory {
	private static final Logger LOG = Log
			.getLogger(CustomResourceServlet.class);
	...
	public void init() throws UnavailableException {
		...
		_contextHandler.setAliases(false);

		



		String rb = Global.getInstance().getAppRootDirectory();
		if (rb != null) {
		...
		int max_cached_file_size =  -2;
		int max_cached_files = -2;
		



		_servletHandler = (ServletHandler) _contextHandler
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			boolean gzip = false;
			if (!included.booleanValue() && _gzip && reqRanges == null
					&& !endsWithSlash) {
			...
			if (LOG.isDebugEnabled())
				LOG.debug("uri=" + request.getRequestURI() + " resource="
				...
				if (_etags) {
					...
					if (ifnm != null && content != null
						...
						if (content
							...
							r.getHttpFields()
									.put(HttpHeaders.ETAG_BUFFER, ifnm);
							return false;
						}
				...
				if (ifms != null) {
					...
					if (ifmsl != -1) {
						if (resource.lastModified() / 1000 <= ifmsl / 1000) {
							...
							r.setStatus(HttpServletResponse.SC_NOT_MODIFIED);
							if (_etags)
								r.getHttpFields().add(HttpHeaders.ETAG_BUFFER,
										content.getETag());
							r.flushBuffer();
							return false;
						}
	...
	protected void sendDirectory(HttpServletRequest request,
		...
		// Otherwise, try using the resource base of its enclosing context
		// handler
	...
	protected void sendData(HttpServletRequest request,
			...
			HttpContent content, Enumeration reqRanges) throws IOException {
		boolean direct;
		long content_length;
		if (content == null) {
			direct = false;
		...
		if (reqRanges == null || !reqRanges.hasMoreElements()
			...
			String mimetype = (content.getContentType() == null ? null
			...
			for (int i = 0; i < ranges.size(); i++) {
				...
				length += ((i > 0) ? 2 : 0)
						...
						+ 2
						+ (mimetype == null ? 0 : HttpHeaders.CONTENT_TYPE
								.length() + 2 + mimetype.length())
						+ 2
	...
	protected void writeHeaders(HttpServletResponse response,
		...
		if (response instanceof Response) {
			...
			if (_etags)
				response.setHeader(HttpHeaders.ETAG, content.getETag()
						.toString());
		}
	}
